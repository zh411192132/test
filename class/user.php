...
$utfdata=0;                                               //数据是否是GBK编码,0为正常数据,1为UTF8编码
...
function admin_EditMember($add,$logininid,$loginin){
	...
	$truepassword=$add[password];//Ucenter
	//修改密码
	$add1='';
	if($add[password])
	{
	...
	//--------------------------- Ucenter ---------------------------
	$ucresult = uc_user_edit($add[username],$oldpassword,$truepassword,$add[email],1);
	if($ucresult == -1)
	{
...
function admin_DelMember($userid,$loginuserid,$loginusername){
	...
	//--------------------------- Ucenter ---------------------------
	$success=uc_user_delete($r[$user_username]);
	if($success==0)
	{
...
function admin_DelMember_all($userid,$logininid,$loginin){
	...
	//--------------------------- Ucenter ---------------------------
	$success=uc_user_delete($delusername);
	if($success==0)
	{
		printerror('删除失败','',1,0,1);
	}
...
function register($username,$password,$repassword,$email){
	...
	if($uid <= 0)
	{
		if($uid == -1)
		{
			printerror('用户名不合法','',1,0,1);
		} 
		elseif($uid == -2)
		{
			printerror('包含要允许注册的词语','',1,0,1);
		} 
		elseif($uid == -3)
		{
			printerror('用户名已经存在','',1,0,1);
		}
		elseif($uid == -4)
		{
			printerror('Email 格式有误','',1,0,1);
		...
		{
			printerror('未定义','',1,0,1);
		}
	}
	@mysql_select_db($GLOBALS['phome_db_dbname']);

	$sql=$empire->query("insert into ".$user_tablename."(".$user_userid.",".$user_username.",".$user_password.",".$user_email.",".$user_registertime.",".$user_group.",".$user_rnd.",".$user_userfen.",".$user_checked.") values('$uid','$username','$password','$email','$registertime','$groupid','$rnd','$public_r[reggetfen]','$checked');");
	//取得userid
	$userid=$uid;//Ucenter
	//附加表
...
function loginout1($userid,$username,$rnd){
	...
	EmptyEcmsCookie();
	esetcookie("mldoactive","",0);//Ucenter
	$dopr=1;
	if($_GET['prtype'])
	{
		$dopr=9;
...
function login1($username,$password,$lifetime,$key,$location){
	global $empire,$user_tablename,$user_userid,$user_username,$user_password,$user_dopass,$user_group,$user_groupid,$user_rnd,$public_r,$user_salt,$user_saltnum,$dbtbpre,$eloginurl,$user_checked,$user_email,$user_registertime,$user_userfen;//Ucenter
	if($eloginurl)
	{
	...
	if($uid <= 0)
	{
		if($uid == -1)
		{
			printerror('用户不存在,或者被删除','',1,0,1);
		}
		elseif($uid == -2)
		{
			printerror('密码错','',1,0,1);
		}
		else
		{
			printerror('未定义','',1,0,1);
	...
	//取得用户资料
	$r=$empire->fetch1("select * from ".$user_tablename." where ".$user_userid."='$uid' limit 1");
	$doactive=0;
	if(!$r[$user_userid])
	{
		//IP
		...
		$registertime=date("Y-m-d H:i:s");
		$checked=1;
		$isql=$empire->query("replace into ".$user_tablename."(".$user_userid.",".$user_username.",".$user_password.",".$user_email.",".$user_registertime.",".$user_group.",".$user_rnd.",".$user_userfen.",".$user_checked.") values('$uid','$username','$password','$email','$registertime','$user_groupid','','$public_r[reggetfen]','$checked');");
		//附加表
		$addr=$empire->fetch1("select * from {$dbtbpre}enewsmemberadd where userid='$uid'");
		if(!$addr[userid])
		{
			$sql1=$empire->query("insert into {$dbtbpre}enewsmemberadd(userid,regip) values('$uid','$regip');");
		}
		$doactive=1;
	}
	$set4=esetcookie("mldoactive","",0);
